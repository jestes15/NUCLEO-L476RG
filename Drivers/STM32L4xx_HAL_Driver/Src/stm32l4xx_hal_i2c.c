                                              ...
                                              uint16_t Size)
{
    ...
    if (hi2c->State == HAL_I2C_STATE_READY)
    {
        ...
        else
        {
            ...
            I2C_TransferConfig(hi2c, DevAddress, (uint8_t)sizetoxfer, I2C_AUTOEND_MODE, I2C_GENERATE_START_WRITE);
...
static HAL_StatusTypeDef I2C_Mem_ISR_IT(struct __I2C_HandleTypeDef *hi2c, uint32_t ITFlags, uint32_t ITSources)
{
             ...
             (I2C_CHECK_IT_SOURCE(ITSources, I2C_IT_TCI) != RESET))
    {
        if ((hi2c->XferCount != 0U) && (hi2c->XferSize == 0U))
        {
            ...
            {
                hi2c->XferSize = MAX_NBYTE_SIZE;
                I2C_TransferConfig(hi2c, (uint16_t)hi2c->Devaddress, (uint8_t)hi2c->XferSize, I2C_RELOAD_MODE,
                                   I2C_NO_STARTSTOP);
            }
            else
            {
                hi2c->XferSize = hi2c->XferCount;
                I2C_TransferConfig(hi2c, (uint16_t)hi2c->Devaddress, (uint8_t)hi2c->XferSize, I2C_AUTOEND_MODE,
                                   I2C_NO_STARTSTOP);
            }
        }
        else
...
static void I2C_DMAMasterReceiveCplt(DMA_HandleTypeDef *hdma)
{
    ...
    else
    {
        ...
        /* Enable the DMA channel */
        if (HAL_DMA_Start_IT(hi2c->hdmarx, (uint32_t)&hi2c->Instance->RXDR, (uint32_t)hi2c->pBuffPtr, hi2c->XferSize) !=
            HAL_OK)
        {
            /* Call the corresponding callback to inform upper layer of End of Transfer */
            I2C_ITError(hi2c, HAL_I2C_ERROR_DMA);
        ...
        while ((__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_STOPF) == RESET) && (status == HAL_OK))
        {
            ...
            if (Timeout != HAL_MAX_DELAY)
            {
                if (((HAL_GetTick() - tickstart) > Timeout) || (Timeout == 0U))
                {
                    ...
                    if ((__HAL_I2C_GET_FLAG(hi2c, I2C_FLAG_BUSY) != RESET) && (tmp1 != I2C_CR2_STOP) &&
